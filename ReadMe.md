...
## `-fno-ipa-pure-const`
From GCC Optimize options, `-fipa-pure-const` "Discover which functions are `pure` or `const`. Enabled by default at -O1 and higher." Turning off this optimization revealed the path to some crashing functions. This was helpful when debugging a leaf function.
